  const fs = require('fs')
  let _limitOrg = JSON.parse(fs.readFileSync('./worker/src/event/database/limit.json'))
require("../../../../connect/config")
  const addInventoriLimit = (sender) => {
        const obj = {id: sender, limit: limitAwal}
         _limitOrg.push(obj)
        fs.writeFileSync('./worker/src/event/database/limit.json', JSON.stringify(_limitOrg))
   }
  ...
  const addLimit = (sender, amount) => {
   ...
   module.exports = { addInventoriLimit, cekDuluJoinAdaApaKagaLimitnyaDiJson, addLimit, kurangLimit, getLimit, _limitOrg}   